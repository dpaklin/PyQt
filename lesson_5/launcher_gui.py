...
from lesson_5.common.variables import DEFAULT_IP_ADDRESS, DEFAULT_PORT
...
class Launcher:
    ...
    def start(self):
        self.processes.append(subprocess.Popen(f'python server.py -p {self.port.get()} -a {self.host.get()}', shell=True, encoding='utf8'))

        for i in range(int(self.clients.get())):
            self.processes.append(subprocess.Popen(f'python client.py {self.host.get()} {self.port.get()} -n test{i + 1}', shell=True, encoding='utf8'))

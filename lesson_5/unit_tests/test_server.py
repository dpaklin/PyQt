...
class TestServer(unittest.TestCase):
    ...
    def test_process_client_message_not_action(self):
        ...
        self.assertEqual(Server.process_client_message(self.message_to_server['not_action']), self.response_err)

    def test_process_client_message_action_not_presence(self):
        """Ошибка если неизвестное действие"""
        self.assertEqual(Server.process_client_message(self.message_to_server['action_not_presence']), self.response_err)

    def test_process_client_message_not_time(self):
        """Ошибка, если  запрос не содержит штампа времени"""
        self.assertEqual(Server.process_client_message(self.message_to_server['not_time']), self.response_err)

    def test_process_client_message_not_user(self):
        """Ошибка - нет пользователя"""
        self.assertEqual(Server.process_client_message(self.message_to_server['not_user']), self.response_err)

    def test_process_client_message_account_name_not_Guest(self):
        """Ошибка - не Guest"""
        self.assertEqual(Server.process_client_message(self.message_to_server['account_name_not_Guest']), self.response_err)
